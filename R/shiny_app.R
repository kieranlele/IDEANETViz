...
### Statistics table ----
  output$statistics_table <-
      renderDT(node_measures)
...
nodelist2 <- reactive({
  if (!is.null(node_data2())) {
    ...
    node_measures <- node_measures %>%
      left_join(node_data3, by = input$node_id_col)
    node_measures
  } else {
    node_measures
  }  
...
#### Setup Role Detection ----
  ...
  output$select_role_viz <- 
    renderUI({
      selectInput('select_role_viz', label = 'Choose Role Output Statistic', choices = c('Empty',
                                                                            'cluster_dendrogram',
                                                                            'concor_block_tree',
                                                                            paste0(input$select_role_type,'_modularity'),
                                                                            paste0(input$select_role_type,'cluster_relations_sociogram'),
                                                                            paste0(input$select_role_type,'cluster_relations_heatmaps'),
                                                                            paste0(input$select_role_type,'cluster_summaries_triad'),
                                                                            paste0(input$select_role_type,'cluster_summaries_cent')
                                                                                         ))
    })
  ...
  output$role_viz <- renderPlot({
    ...
    else if(input$select_role_viz == 'cluster_relations_heatmaps') {
    ...
    else if(input$select_role_viz == 'concor_relations_heatmaps') {
