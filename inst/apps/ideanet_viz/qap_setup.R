...
qap_setup <- function(net, variables, methods, directed = F, additional_vars = NULL) {
  ...
  if (!("id" %in% igraph::list.vertex.attributes(net))) {
...
qap_run <- function(net, dependent = NULL, variables, directed = T, family = "linear", reps = 1000) {
  ...
  if (family == "binomial"){
  # if (all(dv %in% 0:1) == T){
    res <- netlogit(dv, ivs, reps = reps, mode = mode)
  } else if (family == "linear") {
    res <- netlm(dv, ivs, reps = reps, mode = mode)
